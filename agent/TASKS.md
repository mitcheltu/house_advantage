...
- [x] **Multi-sector infrastructure (July 2025):** Fixed bug where Python lists were stringified in `VARCHAR(100)`. Added `_parse_sector()` helper to normalise all DB sector formats. Created `trade_sectors` junction table (9,931 rows, 1,248 multi-sector trades). Cleaned `POLICY_AREA_SECTOR_MAP` (32→27 entries, removed bad mappings). Synced `BILL_SECTOR_MAP` (12→27 entries). API routers now return `sectors: [...]` arrays. Migration script: `backend/db/migrate_trade_sectors.py`. Files modified: `dual_scorer.py`, `merge_trades.py`, `schema.sql`, `setup_db.py`, `contextualizer.py`, `collect_congress_gov.py`, `audit.py`, `politicians.py`, `systemic.py`. 13 new unit tests + 32 regression tests passing.
